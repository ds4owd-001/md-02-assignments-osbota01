...
3.  Do not display (ignore) the outliers in the plot. **Note: Use a search engine or an AI tool to find the solution and add the link to the solution you have found.**
    ...
    Link to solution found using Perplexity: <https://www.perplexity.ai/search/r-ggplotdata-gapminder2007-jV5tJ9woSTS79pHE93mlGQ?s=c>
...
5.  What are the data types of the three variables used for aesthetic mappings?
    ...
    -   continent is categorical, lifeExp is continuous
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: The geom_bar() function creates a bar plot of the number of observations in each category of the x (continent) variable. The geom_col() function requires a y variable to be specified, while geom_bar() calculates the count of observations for the x variable by default.
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    They are similar, but the function geom_histogram() is used for continuous data, while the geom_bar() function is used for categorical data.

7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe: First value is lowest value where your x-axis will start, second value is highest value where your x-axis will end, third value is the width of each bar (new bar for every 2.5 years)
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    The object country_colors stores color schemes for the countries and continents in the Gapminder data.
